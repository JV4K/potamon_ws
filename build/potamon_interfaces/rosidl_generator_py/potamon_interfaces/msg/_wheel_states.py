    ...
    def __import_type_support__(cls):
        try:
            ...
            logger = logging.getLogger(
                'potamon_interfaces.msg.WheelStates')
            logger.debug(
...
class WheelStates(metaclass=Metaclass_WheelStates):
    ...
    def __init__(self, **kwargs):
        ...
        if 'angle' not in kwargs:
            self.angle = numpy.zeros(4, dtype=numpy.float32)
        else:
            self.angle = numpy.array(kwargs.get('angle'), dtype=numpy.float32)
            assert self.angle.shape == (4, )
        ...
        if 'current' not in kwargs:
            self.current = numpy.zeros(4, dtype=numpy.float32)
        else:
    ...
    def angle(self, value):
        if self._check_fields:
            if isinstance(value, numpy.ndarray):
                ...
                assert value.size == 4, \
                    "The 'angle' numpy.ndarray() must have a size of 4"
                self._angle = value
                return
            ...
            assert \
                ...
                "The 'angle' field must be a set or sequence with length 4 and each value of type 'float' and each float in [-340282346600000016151267322115014000640.000000, 340282346600000016151267322115014000640.000000]"
        ...
        if self._check_fields:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -3.402823466e+38 or val > 3.402823466e+38) or math.isinf(val) for val in value)), \
                "The 'current' field must be a set or sequence with length 4 and each value of type 'float' and each float in [-340282346600000016151267322115014000640.000000, 340282346600000016151267322115014000640.000000]"
