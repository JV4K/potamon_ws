...
bool potamon_interfaces__msg__wheel_states__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("potamon_interfaces.msg._wheel_states.WheelStates", full_classname_dest, 48) == 0);
  }
  potamon_interfaces__msg__WheelStates * ros_message = _ros_message;
  {  // angle
  ...
  {  // current
    ...
    }
    {
      ...
      assert(PyArray_TYPE(seq_field) == NPY_FLOAT32);
      Py_ssize_t size = 4;
      float * dest = ros_message->current;
      for (Py_ssize_t i = 0; i < size; ++i) {
        float tmp = *(npy_float32 *)PyArray_GETPTR1(seq_field, i);
        memcpy(&dest[i], &tmp, sizeof(float));
      }
...
PyObject * potamon_interfaces__msg__wheel_states__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("potamon_interfaces.msg._wheel_states");
  ...
  {  // current
    ...
    assert(sizeof(npy_float32) == sizeof(float));
    npy_float32 * dst = (npy_float32 *)PyArray_GETPTR1(seq_field, 0);
    float * src = &(ros_message->current[0]);
    memcpy(dst, src, 4 * sizeof(float));
    Py_DECREF(field);
  }
