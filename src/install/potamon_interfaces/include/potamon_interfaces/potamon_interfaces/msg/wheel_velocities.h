// generated from rosidl_generator_c/resource/idl.h.em
// with input from potamon_interfaces:msg/WheelVelocities.idl
// generated code does not contain a copyright notice

#ifndef POTAMON_INTERFACES__MSG__WHEEL_VELOCITIES_H_
#define POTAMON_INTERFACES__MSG__WHEEL_VELOCITIES_H_

#include "potamon_interfaces/msg/detail/wheel_velocities__struct.h"
#include "potamon_interfaces/msg/detail/wheel_velocities__functions.h"
#include "potamon_interfaces/msg/detail/wheel_velocities__type_support.h"

#endif  // POTAMON_INTERFACES__MSG__WHEEL_VELOCITIES_H_
